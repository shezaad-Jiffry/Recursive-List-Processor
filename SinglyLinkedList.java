...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    public int count(E fromElement, E toElement) {
        Node<E> temp = head;//temporary for the head as to not mess with the order
        Boolean found = false;//boolean that controlls whether we add to the return or not (only add when fromelement is found)
        //BASE CASE
        //if the fromelement and to element match then you must return 1
        if(fromElement == toElement){
            return 1;
        }
        //RECURSIVE METHOD CALL
        return count(temp, fromElement, toElement, found);
    }

    private int count(Node<E> node, E fromElement, E toElement, Boolean found){
        if(node != null){
            //BASE CASE
            //once toelement is found and from element is found then we can properly return
            if(node.value == toElement && found){
                return 1;
            }
            //PRE PROCESSING
            //only add when we found from element then we add there on out setting found to true
            if(node.value == fromElement || found){
                return 1 + count(node.next,fromElement,toElement,true);
            }
            //GENERAL
            //basically just loop back to the next node adding nothing 
            else{
                return count(node.next,fromElement,toElement,false);
            }
        }
        //if from and to element are never found by the end just return one last time adding nothing
        return 0;
    }
